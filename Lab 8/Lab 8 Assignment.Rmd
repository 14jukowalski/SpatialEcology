...
```
I chose a threshold value of 0.25 because I wanted to be able to visualize the patches and using a threshold of 0.5 had very few patches so I chose 0.25 for this exercise. In practice a threshold value could be used when you want to include as much area where varied thrush could be found as possible, but it would not be a very practical threshold in most situations. The total amount of habitat and the patch size of habitat is much larger for the mesic forest approach than for the SDM-based approach. You may rely on the mesic forest approach if you do not have the resources to sample for the species and create a SDM or if you are just looking for a general area where the specie may be located, for example selecting sample sites to collect varied thrush when little data exists suggesting where they might be located. You may rely on the SDM-based approach if you are interested in showing where the varied thrush has the highest probability may be located, for example if trying to create management areas or areas to protect the varied thrush. 
